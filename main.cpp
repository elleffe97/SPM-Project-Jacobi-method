...
int main(int argc, char * argv[]){
	...
	if (argv[1]==NULL){						//number of threads
		arg[0]=4;
		arg[1]=1000;
		arg[2]=500;
	}						
	...
	if (argv[2]==NULL){						//matrix dimension
		arg[1]=1000;
		arg[2]=500;	
	}
	else{
	...
	if (argv[3]==NULL || atoi(argv[3])<=1)	//number of iterations
		arg[2]=500;								
	else
		arg[2]=atoi(argv[3]);		
	...
	int NrIter;					//variable for the real number of iterations computed
	...
	vector<float> xser=serial(arg, A, b, &tseq, &NrIter);			//result of sequential algorithm
	...
	float Effpar=efficiency(tseq, tparn, nt);				//efficiency of the thread algorithm
	...
	float Effff=efficiency(tseq, tff, nt);					//efficiency of the fastflow algorithm
	...
	cout<< "Do you want to see the result and the number of iterations computed? (Y/N)"<< endl;
	...
	if (r=='Y' || r=='y'){
		fprint(xser);
		cout<<endl<<"Computed with "<<NrIter<<" iterations."<<endl<<endl;
